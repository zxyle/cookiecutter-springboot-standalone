...
target_dir = os.path.join(stage_dir, "{{cookiecutter.artifact}}")
...
def replace_content(input_content):
    # 替换掉包名
    after_replace_base_package = input_content.replace(base_package, "{{ cookiecutter.basePackage }}")
    # 替换掉版本号、作者和生成日期
...
def move_all_files(files):
    for line in files:
        ...
        target_file_abs_path = os.path.join(target_dir,
                                            filename.replace(package_path, "{{ cookiecutter.package_path }}"))

...
public class WechatAuthenticationSuccessHandler implements AuthenticationSuccessHandler {
    ...
    public void setStringRedisTemplate(StringRedisTemplate stringRedisTemplate) {
        WechatAuthenticationSuccessHandler.stringRedisTemplate = stringRedisTemplate;
    }

    @Override
    public void onAuthenticationSuccess(HttpServletRequest request, HttpServletResponse response, Authentication authentication) throws IOException {
        LoginUser principal = (LoginUser) authentication.getPrincipal();
        request.setAttribute(WechatAuthenticationFilter.USER_ID, principal.getUser().getId());

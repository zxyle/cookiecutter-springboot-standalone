// This project was generated by {{ cookiecutter.author_name }} at {% now 'local', '%Y-%m-%d %X' %}.
// The version of generator is v{{ cookiecutter._version }}.

package {{ cookiecutter.basePackage }}.biz.auth.mapper;

import {{ cookiecutter.basePackage }}.biz.auth.entity.Profile;
import com.baomidou.mybatisplus.core.mapper.BaseMapper;
import org.apache.ibatis.annotations.Select;
import org.apache.ibatis.annotations.Update;
import org.springframework.stereotype.Repository;

/**
 * 用户信息 Mapper 接口
 */
@Repository
public interface ProfileMapper extends BaseMapper<Profile> {

    // 截断表
    @Update("TRUNCATE TABLE auth_profile")
    void truncate();

    // 创建分区表
    @Select("CREATE TABlE IF NOT EXISTS auth_profile_${suffix} LIKE auth_profile;")
    void cloneTable(String suffix);

    // 删除分区表
    @Select("DROP TABlE IF EXISTS auth_profile_${suffix} ;")
    void dropTable(String suffix);

}
